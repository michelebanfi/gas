...
export class MarkerManager {
  ...
  renderMarkers(fuelData, selectedFuelTypes) {
    ...
    fuelData.features.forEach((feature) => {
      ...
      const matchingFuels = selectedFuelTypes.filter(
        (fuelType) => prices && prices[fuelType] !== undefined,
      );
    ...
    this.map.addLayer({
      ...
      paint: {
        "circle-color": [
          "step",
          ["get", "point_count"],
          "rgba(99, 102, 241, 0.7)", // Indigo for small clusters
          10,
          "rgba(99, 102, 241, 0.85)", // Darker indigo for medium clusters
          30,
          "#6366f1", // Full indigo for large clusters
        ],
        "circle-radius": [
          "step",
          ["get", "point_count"],
  ...
  _updateHTMLMarkers() {
    ...
    visibleIndices.forEach((index) => {
      ...
      if (this.searchCenter && this.searchRadius) {
        ...
        const distance = turf.distance(this.searchCenter, stationPoint, {
          units: "kilometers",
        });
        markerObj.element.style.opacity =
          distance <= this.searchRadius ? "1" : "0.3";
      } else {
    ...
    if (this.searchCenter && this.searchRadius) {
      ...
      clusters.forEach((cluster) => {
        if (cluster.properties.cluster) {
          ...
          const distance = turf.distance(this.searchCenter, clusterPoint, {
            units: "kilometers",
          });
  ...
  clearMarkers() {
    ...
    const layers = ["fuel-cluster-count", "fuel-clusters"];
